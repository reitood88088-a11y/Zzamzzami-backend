...
def get_quizzes(
    ...
    try:
        ...
        words = session.exec(select(Word).where(Word.diary_id.in_(diary_ids))).all()
        if not words:
            return {"success": True, "data": []}
        ...
        prompt = f"""
        You are an AI assistant for a language learning app. Language: {subject if subject != 'ALL' else 'Mixed'}
        ...
        # If loop finished but no data and last_exception exists, raise it to trigger the mock fallback
        if not new_quizzes_data and last_exception:
            raise last_exception
        
        latest_diary = sorted(recent_diaries, key=lambda d: d.created_at, reverse=True)[0]
        ...
        new_quizzes = session.exec(select(Quiz).where(Quiz.diary_id == latest_diary.id)).all()
        ...
        print(f"Gemini API or Model Error: {e}. Using fallback generation.")
        # Fallback to generating quizzes locally based on extracted words
        import random
        fallback_quizzes = []
        if len(word_list) > 0:
        ...
        for qd in fallback_quizzes:
            new_quiz = Quiz(
                diary_id=recent_diaries[0].id,
                question=qd['question'],
                options=qd['options'],
                correct_option_index=qd['correct_option_index'],
                explanation=qd['explanation']
        ...
        new_quizzes = session.exec(select(Quiz).where(Quiz.diary_id == recent_diaries[0].id)).all()
        return {
            "success": True,
            "data": [
                {
                    "quizId": str(quiz.id),
                    "diaryId": str(recent_diaries[0].id),
                    "question": quiz.question,
                    "options": quiz.options,
                    "correctOptionIndex": quiz.correct_option_index,
                    "explanation": quiz.explanation
    ...
    # We removed the if "429" check, so it always falls back. We don't need the else block anymore.
    # It just returns the fallback quizzes.
